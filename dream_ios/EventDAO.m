...
-(void) collectEvent:(drEvent*)model
{
    ...
    NSString *str = [NSString stringWithFormat:@"%@/%@",HOST_NAME,@"json/collectEvent.action"];
    NSURL *url = [NSURL URLWithString:[str URLEncodedString]];
    NSLog(@"url-->%@",[url description]);
    __weak ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    ...
    [request setCompletionBlock:^{
        ...
        if ([resultCodeNumber integerValue] ==0)
        {
            [self.delegate collectEventFinished:model];
        } else {
            NSInteger resultCode = [resultCodeNumber integerValue];
            NSString* message = [resDict objectForKey:@"error_message"];
            NSDictionary *userInfo = [NSDictionary dictionaryWithObject:message
                                                                 forKey:NSLocalizedDescriptionKey];
            NSError *err = [NSError errorWithDomain:@"DAO" code:resultCode userInfo:userInfo];
            [self.delegate collectEventFailed:err];
        }
    }];
    [request setFailedBlock:^{
        NSError *error = [request error];
        [self.delegate collectEventFailed:error];
    }];
    [request startAsynchronous];
...
-(void) getUserCollection:(drEvent*)model
{
    ...
    NSString *str = [NSString stringWithFormat:@"%@/%@",HOST_NAME,@"json/getUserCollection.action"];
    NSURL *url = [NSURL URLWithString:[str URLEncodedString]];
    NSLog(@"url-->%@",[url description]);
    __weak ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    ...
    [request setCompletionBlock:^{
        ...
        if ([resultCodeNumber integerValue] ==0)
        {
            ...
            [self.delegate getUserCollectionFinished:listData];
        } else {
            NSInteger resultCode = [resultCodeNumber integerValue];
            NSString* message = [resDict objectForKey:@"error_message"];
            NSDictionary *userInfo = [NSDictionary dictionaryWithObject:message
                                                                 forKey:NSLocalizedDescriptionKey];
            NSError *err = [NSError errorWithDomain:@"DAO" code:resultCode userInfo:userInfo];
            [self.delegate getUserCollectionFailed:err];
        }
    }];
    [request setFailedBlock:^{
        NSError *error = [request error];
        [self.delegate getUserCollectionFailed:error];
    }];
    [request startAsynchronous];
}
@end
